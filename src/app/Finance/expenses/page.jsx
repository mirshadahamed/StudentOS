  ...
  const fetchExpenses = async () => {
    try {
      const [txRes, streakRes] = await Promise.all([
        ...
        fetch(withUserQuery('/api/finance/streak')) 
      ]);
                ...
                <button 
                  onClick={async () => {
                    ...
                    await fetch('/api/finance/transactions', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify(withUserBody({ title: wizardInput.title, amount: parseFloat(wizardInput.amount), type: 'expense', category: wizardCategories[wizardStep], status: 'completed' })),
                    });
                    await fetchExpenses();
                    setWizardInput({title: '', amount: ''});
                  }}
