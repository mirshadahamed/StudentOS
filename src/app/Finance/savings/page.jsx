...
export default function SavingsPage() {
  ...
  const fetchSavings = async () => {
    try {
      const res = await fetch(withUserQuery('/api/finance/savings'));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await fetch('/api/finance/savings', {
        ...
        body: JSON.stringify(withUserBody({
          name: formData.name,
  ...
  const handleAddFunds = async (e) => {
    ...
    try {
      await fetch(withUserQuery(`/api/finance/savings/${selectedJar.id}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(withUserBody({ amountToAdd: parseFloat(addAmount) }))
      });
      await fetchSavings();
