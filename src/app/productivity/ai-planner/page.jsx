...
export default function AIPlannerPage() {
  ...
  const INIT_MSG = { role:'assistant', content:`Hi ${userName}. I can see your tasks and their AI scores. Ask me what to work on first, how to plan your week, or anything about your deadlines.` };
  ...
  useEffect(()=>{
    const load = async () => {
      ...
      const [t, s, profile] = await Promise.all([
      ...
      if (sorted.length) {
        setMessages([{ role:'assistant', content:
          `Hi ${profile?.name || 'Student'}. I can see ${sorted.length} active task${sorted.length!==1?'s':''}. Top priority right now: "${sorted[0].title}" — AI score ${sorted[0].aiScore||0}/100, ${sorted[0].daysLeft??'TBD'} days left, ${sorted[0].progress}% done. What would you like to focus on?`
        }]);
      }
    };
    load();
