  ...
  <motion.div initial={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:0.9 }}
    ...
    <motion.div initial={{ scale:0.82, opacity:0 }} animate={{ scale:1, opacity:1 }}
      ...
      <h1 className="text-5xl font-black tracking-[0.45em] mb-4">
        <span style={{ background:'linear-gradient(135deg,#f97316,#ec4899)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>STUDENT</span>
...
const NavCard = ({ icon:Icon, label, sub, href, delay }) => (
  <Link href={href} className="w-full">
    <motion.div initial={{ opacity:0, y:18 }} animate={{ opacity:1, y:0 }}
      ...
      style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(249,115,22,0.15)', backdropFilter:'blur(12px)' }}
      ...
      <div className="flex justify-between items-start">
        ...
        <ArrowUpRight size={14} className="opacity-20 group-hover:opacity-80 transition-all" style={{ color:'#f97316' }}/>
      </div>
      <div>
...
const AddTaskForm = ({ onAdded }) => {
  ...
  const submit = async (e) => {
    ...
    const cleanSubs = subtasks.filter(s => s.trim()).map(s => ({ title: s.trim(), done: false }));

    const result = await tasksAPI.create({ ...form, title:form.title.trim(), subtasks: cleanSubs });
    setSaving(false);
            ...
            <div className="space-y-4 pt-4" style={{ borderTop:'1px solid rgba(249,115,22,0.1)' }}>
              ...
              <div>
                ...
                <button type="button" onClick={addSubRow}
                  className="mt-2.5 flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider transition-all hover:text-orange-400"
                  style={{ color:'rgba(249,115,22,0.5)' }}>
                  <Plus size={11}/> Add another step
                </button>
                <p className="text-[10px] mt-2 text-neutral-700">
                  Steps are optional. If you add them, task progress is calculated from how many steps you complete.
                </p>
              </div>
    ...
    <div className="min-h-screen relative text-white" style={{ fontFamily:'system-ui,sans-serif' }}>
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-8 py-16 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="rounded-[2rem] p-7 flex items-center gap-5" style={{ background:'rgba(251,146,60,0.06)', border:'1px solid rgba(251,146,60,0.15)' }}>
          ...
          <div className="rounded-[2rem] p-7 flex items-center gap-5" style={{ background:'rgba(236,72,153,0.06)', border:'1px solid rgba(236,72,153,0.15)' }}>
